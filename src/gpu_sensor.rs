...
fn generate_json(device: nvml_wrapper::Device) -> SensorDataValues {
    ...
    let sensor_data = SensorValues{value_type: "gpu_load".to_string(), value: utilization_rates};
    result.values.push(sensor_data);

    let memory_info = match gpustat.memory_info {
        Ok(memory_info) => format!(
            "{:.6}", (memory_info.used as f64 / memory_info.total as f64).to_string()
        ),
        Err(_err) => "".to_string(),
    };
    let sensor_data = SensorValues{value_type: "gpu_memory".to_string(), value: memory_info};
    result.values.push(sensor_data);
    ...
    let sensor_data = SensorValues{value_type: "gpu_temperature".to_string(), value: temperature};
    result.values.push(sensor_data);
